...
export default function ResourceAllocation() {
  ...
  const [centerData, setCenterData] = useState<CenterData>({ staffCount: 0, travelTimeMin: 30 });
  ...
  const handleOptimize = async () => {
    ...
    try {
      // Build zones dict from report_zones table
      const { data: reportZones, error: zonesError } = await supabase
        .from("report_zones")
        .select("zone_name, firebreak_area_m2")
        .eq("report_id", state!.reportId);

      if (zonesError) throw zonesError;

      const zones: Record<string, number> = {};
      reportZones?.forEach((rz) => {
        if (rz.firebreak_area_m2 > 0) {
          zones[rz.zone_name] = rz.firebreak_area_m2;
        }
      });

      if (Object.keys(zones).length === 0) {
        toast({ title: "No zone data", description: "No zones with firebreak area found", variant: "destructive" });
      ...
      console.log("Zones payload:", JSON.stringify(zones, null, 2));
      console.log("Centers payload:", JSON.stringify(centers, null, 2));
      console.log("Expected N_max (TeamSize=7):", Math.floor(centerData.staffCount / 7));
      console.log("==============================");

      const response = await runOptimization(zones, centers);
